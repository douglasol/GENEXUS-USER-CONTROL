...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'BootstrapCarousel'">
        <xsl:call-template name="RenderBootstrapCarousel"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- BootstrapCarousel design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderBootstrapCarousel">
    <span atomicselection="true">
      <xsl:call-template name="AddStyleAttribute"/>
			<div style="border:2px solid #5d4384;">
			    ...
			    <p style="color:#5d4384;padding:5px; font-family: Helvetica,Arial,sans-serif ;font-size: 20px;">Carousel - <xsl:value-of select="gxca:GetStringPropertyValue('ControlName')"/></p>
			</div>
    </span>
  </xsl:template>
